...
function InstallationsTable() {
  ...
  return (
    <div className="bg-white rounded shadow p-6 mb-8">
      <h2 className="text-xl font-bold mb-4 text-center">جدول التركيبات</h2>
      <button
        ...
        <table className="w-full border text-center">
          ...
          <tbody>
            {installations.map((i, idx) => (
              <tr key={i.InstallationID || idx} className="hover:bg-gray-50">
                <td className="p-2 border">{idx + 1}</td>
                <td className="p-2 border">{i.OrderID}</td>
                <td className="p-2 border">{i.ClientID}</td>
                <td className="p-2 border">{i.Status}</td>
                <td className="p-2 border">{i.AppointmentDate}</td>
                <td className="p-2 border">{i.AssignedTeam}</td>
                <td className="p-2 border">{i.ContractNumber}</td>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border-2 border-blue-100 animate-fadeIn flex flex-col max-h-[95vh]">
            <h2 className="text-xl font-bold mb-2 text-center text-blue-800 sticky top-0 bg-white z-10 pt-6">
              {editId ? 'تعديل تركيب' : 'إضافة تركيب جديد'}
            </h2>
